...
**Abordagem técnica:** Arquitetura em camadas combinando (1) pré-processamento com normalização Unicode, (2) padrões regex otimizados para CPF, RG, telefone e e-mail brasileiros, com filtros anti-falso-positivo para SEI/NUP, e (3) modelo BERTimbau NER (HuggingFace) para detecção de nomes de pessoas físicas, com lista de 130+ exclusões institucionais.

**Estratégia recall-first:** Prioriza minimizar falsos negativos conforme critérios de desempate do edital. Nos testes com a amostra oficial de 99 registros, obteve 100% de recall (zero FN), precisão de 96,7% e F1-Score estimado de 0,98.

**Diferencial inovador:** Sistema de Revisão Humana integrado que identifica automaticamente casos ambíguos (contextos artísticos, acadêmicos, jurídicos) e gera relatório priorizado com fundamentação na LGPD, permitindo verificação manual eficiente sem comprometer a sensibilidade do detector.

**Tecnologias:** Python 3.9+, Transformers (HuggingFace), BERTimbau NER, regex, pandas.

**Uso de IA:** Desenvolvimento assistido por Claude Code (Anthropic) conforme item 13.9 do edital.
